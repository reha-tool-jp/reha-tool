...
# We need to replace the top common CSS with just <link ...> <style> and keeping the rest.
# The top common CSS ends exactly at `.breadcrumb span { margin: 0 6px; }`
top_target = """<style>
:root {
  --bg: #f5f4f0;
  --surface: #ffffff;
...
top_replacement = """<link rel="stylesheet" href="css/style.css">
<style>"""

content = content.replace(top_target, top_replacement)

# The bottom common CSS starts at `.footer {` and ends at `}</style>`
bottom_target = """.footer { text-align: center; font-size: 12px; color: var(--text3); margin-top: 40px; line-height: 2.2; }
...
.nav-container a:hover { background: var(--accent-light); color: var(--accent); }</style>"""

bottom_replacement = """</style>"""
